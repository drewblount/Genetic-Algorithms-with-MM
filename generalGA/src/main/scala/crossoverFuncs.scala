...
object crossoverFuncs {       
  ...
  def sliceFunc[A](parentA: Typed[A], parentB: Typed[A],
      		   crossoverProb: Double):(Typed[A],Typed[A]) = {
  ...
  def shuffleFunc[A](parentA: Typed[A], parentB: Typed[A],
                  crossoverProb: Double): (Typed[A], Typed[A]) = {
    ...
    val childA:Typed[A] = parentA
    val childB:Typed[A] = parentB
